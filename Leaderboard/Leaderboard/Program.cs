...
namespace Leaderboard
{
    class Program
    {
        ...
        public static List<int> climbingLeaderboard(List<int> ranked, List<int> player)
        {
            ...
            List<int> rankedCleaned = ranked.Distinct<int>().ToList();

            int start = rankedCleaned.Count();
            foreach (var item in player)
            {
                var added = false;                
                for (int i = start - 1; i >= 0; i--)
                {
                    if (item < rankedCleaned[i])
                    {
                        output.Add(i+2);
                        added = true;
                        start = i+1;
